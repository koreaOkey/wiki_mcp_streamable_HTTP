...
**✨ Streamable HTTP Transport 지원** - MCP 표준 HTTP 통신 방식을 사용합니다.
...
| 환경 변수 | 필수 | 기본값 | 설명 | 예시 |
|-----------|------|--------|------|------|
| `MCP_TRANSPORT` | ❌ | `streamable` | 통신 방식 | `streamable` / `stdio` |
| `MCP_HOST` | ❌ | `0.0.0.0` | 리스닝 IP (외부 접속 허용) | `0.0.0.0` / `192.168.1.200` |
| `MCP_PORT` | ❌ | `8002` | 서비스 포트 | `8002` / `8080` |

💡 **통신 방식:**
- `streamable`: HTTP Streamable Transport (권장) - 표준 MCP HTTP 통신
- `stdio`: 표준 입출력 방식
...
uv run wiki-mcp --transport streamable --port 8002
...
- **Docker 배포**: 동일한 방식으로 사용 가능

#### 🎯 장점
- ✅ MCP 표준 프로토콜 준수
- ✅ 더 나은 호환성 및 확장성
- ✅ 비동기 처리로 성능 향상 가능
- ✅ RESTful API 엔드포인트 제공 (`/health`, `/`)
